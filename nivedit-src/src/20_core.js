...
const VER = 'V14';          // 每次更新都會變，用來確認瀏覽器有沒有載到新版
const VER_DATE = '2026/09/25';
...
function tagSub(c, L, preferred){
  L = L || layout();
  c.cid = null; c.cs = 0;
  // 字幕跟它那一軌的影片是一組，所以【優先】綁在自己這一軌上的片段。
  // 少了這一步，把字幕拖到另一條字幕軌之後它還是綁著原本那一軌的影片，
  // 下一次同步就會被拉回去 —— 看起來像「拖不動」。
  const st=subTrack(c);
  const indices=L.map((_,i)=>i).reverse()
    .sort((a,b)=>(clipTrack(A.clips[b])===st)-(clipTrack(A.clips[a])===st));
...
/** 排版指紋：syncSubsToClips 靠它判斷「排版有沒有變」。

    軌道一定要記進去。少了它的話，「同一個時間、換一條軌」算出來的指紋跟之前一樣，
    整個字幕重算就不會被觸發 —— 影片搬到上軌了，字幕卻留在下軌。
    （v10.3 剛修好字幕跟著換軌，卻被這個指紋擋在門外，白修。） */
function layoutSig(L){
  L = L || layout();
  return A.clips.map((c, i) => c.id + '@' + L[i].start.toFixed(4) + '+' + L[i].dur.toFixed(4)
                               + '#' + c.inP.toFixed(4) + '/' + L[i].track).join('|');
}

/** 排版變了就把字幕搬回原來對到的畫面上。
    自己會分辨「使用者手動拖過的字幕」（start 跟上次記的不一樣），那種就重新記位置，
    所以不用在每個會動到字幕的地方都補一行呼叫。 */
/** 開檔專用：把字幕拉回它綁的那一段所在的影片軌。

    只在載入專案時跑一次。平常的重繪【不能】做這件事 —— 試過，
    那會把使用者手動選的字幕軌在下一次重繪硬扳回去，等於廢掉分軌功能。

    存在的理由：v10.3 以前搬過軌的專案會留下「影片在上軌、字幕在下軌」，
    那種狀態下字幕會被它自己綁的那段影片蓋住。開檔時一次修好，
    不必要求使用者手動再搬一次片段。圖片軌沒有對應的字幕軌，不動。 */
function alignSubTracksToClips(){
  let n = 0;
  for (const c of A.subs){
    if (!c.cid) continue;
    const i = A.clips.findIndex(x => x.id === c.cid);
    // 連動的那一段不在了就先跳過。這裡【不要】順手 tagSub 重綁 ——
    // 試過，會連帶把匯入、分割、存讀那幾條路的字幕歸屬打亂（三支測試同時紅）。
    // 殘留一個用不到的 id 是內部細節，使用者看不到。
    if (i < 0) continue;
    const ct = clipTrack(A.clips[i]);
    // 只修【一個方向】：字幕比它綁的影片還低，會被自己那段影片蓋住 —— 那是壞掉。
    // 反過來（字幕在上軌、影片在下軌）照樣看得見，是使用者刻意的擺法，不要動它。
    if (ct !== IMG_TRACK && subTrack(c) < ct){ c.track = ct; n++; }
  }
  return n;
}
...
function syncSubsToClips(){
  ...
  A._laySig = sig;
  let moved = 0;
  // 這一輪有哪些片段「自己換了軌」。字幕只在這種時候跟著換軌，
  // 其他情況（例如同軌左右拖）一律不碰字幕的軌道。
  // _tr 只活在記憶體裡，serialize 的欄位白名單沒有它，不會進 .nvproj。
  const trackMoved = new Map();
  for (const cl of A.clips){
    const t = clipTrack(cl);
    if (cl._tr !== undefined && cl._tr !== t) trackMoved.set(cl.id, t);
    cl._tr = t;
  }
  for (const c of A.subs){
    ...
    /* 搬動某一段影片時，連動它的字幕一定要跟著搬 —— 包含換軌。

       但【只有那一段自己換軌】才動字幕。不能寫成「字幕永遠跟它綁的影片同軌」：
       只有一條影片軌、卻用了兩條字幕軌（下軌 AI 字幕、上軌自己加的）是既有用法，
       那些上軌字幕會依時間綁到下軌的影片，寫成永遠同軌就會把它們整批拖下來，
       等於廢掉分軌字幕。試過，subtitle-scope 與 subtitle-edit 當場紅。
       圖片軌沒有對應的字幕軌，不動。 */
    const nt = trackMoved.get(c.cid);
    if (nt !== undefined && nt !== IMG_TRACK && subTrack(c) !== nt){ c.track = nt; moved++; }
