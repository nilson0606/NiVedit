...
const ASR_MODELS = [
  ...
  { id:'onnx-community/whisper-large-v3-turbo',  name:'最準（large-v3-turbo，GPU 約 1.6GB）' },
...
self.addEventListener('unhandledrejection', e => {
  post('error', { msg: '辨識引擎發生錯誤：' + String(e.reason && e.reason.message || e.reason) });
});
...
async function getPipe(model, device){
  const adapter = device === 'webgpu' ? await gpuAdapter() : null;
  if (device === 'webgpu' && !adapter){
    device = 'wasm';
  ...
  if (pipe && curKey === key){
    post('plan', { name: pipe.__nvPlan, device: pipe.__nvDevice });
    return pipe;
  }
  ...
  const plans = [];
  // Turbo FP32 needs separate 2.55GB weights; do not select it in a browser.
  const turbo = model === 'onnx-community/whisper-large-v3-turbo';
  if (device === 'webgpu'){
    if (turbo){
      if (adapter.features.has('shader-f16')) plans.push({ device:'webgpu', dtype:'fp16' });
      plans.push({ device:'webgpu', dtype:{ encoder_model:'q8', decoder_model_merged:'q4' } });
    } else {
  ...
  if (!turbo) plans.push({ device:'wasm', dtype:'fp32' });
  ...
  for (const pl of plans){
    try {
      ...
      curKey = key;
      pipe.__nvPlan = name(pl);
      pipe.__nvDevice = pl.device;
...
self.onmessage = async e => {
  ...
  try {
    ...
    let done = 0;
    // Restore generate after each run; reused pipelines must not keep old counters.
    const model = p.model, originalGenerate = model.generate;
    model.generate = async (...a) => {
      const r = await originalGenerate.apply(model, a);
    ...
    } finally { model.generate = originalGenerate; }
  } catch(err){
...
function asrWorker(){
  ...
  ASR.worker._nvURL = url;
  return ASR.worker;
}
function asrKillWorker(worker = ASR.worker){
  if (!worker) return;
  try { worker.terminate(); } catch(e){}
  if (worker._nvURL) URL.revokeObjectURL(worker._nvURL);
  if (ASR.worker === worker) ASR.worker = null;
}
...
const ASR_MUSIC_PREFIX = 'music:';
/** 範圍是不是某一條音軌。影片片段與音軌的 id 都是 uid()，光看值分不出來，
    所以下拉的 value 給音軌加前綴。 */
const asrMusicOf = scopeId => (typeof scopeId === 'string' && scopeId.startsWith(ASR_MUSIC_PREFIX))
  ? A.musics.find(m => m.id === scopeId.slice(ASR_MUSIC_PREFIX.length)) || null : null;
...
async function asrExtract16k(scopeId, onProg){
  ...
  /* ── 音軌（v12.0）───────────────────────────────────────────
     ...
     只辨識【聽得到的那一段】：從檔案的 m.offset 開始，長度取
     min(m.len, musicSeg(m))。理由有兩個 ——
       · 超過 m.len 的部分時間軸上根本不播，給它字幕等於憑空多出句子；
       · 開了循環時後面是同一段重播，整段辨識會產生一模一樣的重複字幕。
     所以循環的音軌只辨識第一輪，這是刻意的。 */
  if (typeof scopeId === 'string' && scopeId.startsWith(ASR_MUSIC_PREFIX) && !asrMusicOf(scopeId))
    throw new Error('找不到那一條音軌（可能已經被刪掉了）');
  const mus = asrMusicOf(scopeId);
  if (mus){
    const d = Math.max(0.1, Math.min(mus.len, musicSeg(mus)));
    if (d > 3 * 3600) throw new Error('超過 3 小時，記憶體會爆，請分段處理');
    const oc = new OfflineAudioContext(1, Math.ceil(d * SR), SR);
    if (onProg) onProg(`解碼「${mus.name}」…`);
    const buf = await getClipAudio(mus, oc);       // music 也有 .file／.name，同一條路
    if (!buf) throw new Error(`解不出「${mus.name}」的聲音`);
    const src = oc.createBufferSource();
    src.buffer = buf;
    src.connect(oc.destination);
    src.start(0, Math.min(mus.offset, Math.max(0, buf.duration - 0.05)),
              Math.min(d, Math.max(0, buf.duration - mus.offset)));
    if (onProg) onProg('混音中…');
    const out = await oc.startRendering();
    return { pcm: out.getChannelData(0), offset: mus.startAt, dur: d };
  }
...
async function asrRun(){
  ...
  $('#mCancel').onclick = () => {
    ASR.cancel = true;
    if (abortRun) abortRun(); else asrKillWorker();
    mDone('已取消', '', '—');
  };
  try {
    mProg(2, '抽出聲音…');
    ...
    const note = x => { trace.push(x); if (trace.length > 100) trace.shift(); };
    note('模型：' + ASR.model);
    ...
    const chunks = await new Promise((res, rej) => {
      ...
      const fin = (fn, v) => {
        ...
        clearTimeout(wd); abortRun = null;
        fn(v);
      };
      const fail = error => { if (settled) return; asrKillWorker(w); fin(rej, error); };
      abortRun = () => fail(new Error('已取消'));
      w.onerror = e => { e.preventDefault(); fail(new Error('辨識工作程序失敗：' + (e.message || '未知錯誤'))); };
      w.onmessageerror = () => fail(new Error('辨識工作程序訊息無法讀取'));
      const kick = () => { clearTimeout(wd); wd = setTimeout(() => {
        fail(new Error('五分鐘未收到進度，已中止。最後階段：' + lastStage +
          '。這不代表顯示卡效能不足；請查看過程紀錄中的模型與載入方式。'));
      }, 300000); };
      kick();
      w.onmessage = ev => {
        const m = ev.data;
        if (settled) return;
        kick();
        if (m.t === 'status'){ lastStage = m.s; note(m.s); mProg(Math.max(8, dlPct), m.s); }
          ...
          if (m.status === 'initiate' && m.file){
            lastStage = '讀取模型檔 ' + m.file; note(lastStage);
          }
          if (m.status === 'progress' && m.total){
            lastStage = '讀取模型檔 ' + m.file;
            ...
            note('模型檔已讀取：' + m.file);
            lastStage = '已讀取 ' + m.file + '；準備其餘檔案與初始化模型…';
            mProg(Math.max(dlPct, 38), lastStage);
          }
        }
        else if (m.t === 'step'){
          lastStage = '辨識中 ' + m.done + ' / ' + m.total + ' 段';
          const p = 40 + Math.round(Math.min(1, m.done / Math.max(1, m.total)) * 55);
        ...
        else if (m.t === 'error'){ note('✗ ' + m.msg); fail(new Error(m.msg)); }
      };
      try {
    ...
    /* 落在哪一條字幕軌：影片片段跟著它自己那一軌；
       音軌沒有對應的影片軌，一律放底層（clipTrack(undefined) 也是 0，
       但這裡寫清楚，免得下次看到以為是漏掉的分支）。 */
    cues.forEach(c => c.track = asrMusicOf(ASR.scope)
      ? 0 : clipTrack(A.clips.find(x => x.id === ASR.scope)));
...
function asrPanel(){
  const box = $('#asrbox');
  if (!box) return;
  /* 選好的範圍被刪掉了就落回「整支影片」。不歸位的話下拉會顯示第一個選項，
     ASR.scope 卻還指著不存在的東西 —— 看起來選了 A，按下去辨識 B。 */
  if (ASR.scope && ASR.scope !== 'all'
      && !(asrMusicOf(ASR.scope) || A.clips.some(c => c.id === ASR.scope)))
    ASR.scope = 'all';
  const clipOpts = ['<option value="all">整支影片（時間軸全部）</option>']
    .concat(A.clips.map((c, i) => `<option value="${c.id}"${ASR.scope === c.id ? ' selected':''}>第 ${i+1} 段：${esc(c.name)}</option>`))
    .concat(A.musics.length
      ? ['<optgroup label="音軌">'].concat(A.musics.map((m, i) =>
          /* 用破折號不用全形冒號：翻譯是逐詞比對的，「音軌」有詞條、「：」沒有，
             用冒號的話英文會變成「Audio 1：name」，中間卡一個全形冒號。 */
          `<option value="${ASR_MUSIC_PREFIX}${m.id}"${ASR.scope === ASR_MUSIC_PREFIX + m.id ? ' selected':''}>音軌 ${i+1} — ${esc(m.name)}</option>`))
          .concat(['</optgroup>'])
      : []).join('');
  box.innerHTML = `
    <div class="row"><label>範圍</label><div class="f"><select id="asrScope" style="flex:1">${clipOpts}</select></div></div>
    ${asrMusicOf(ASR.scope) ? `<div class="hint" style="margin:-3px 0 7px">
      只辨識這條音軌在時間軸上聽得到的那一段，產生的字幕放在字幕底層。<br>
      開了循環的話只辨識第一輪，不會跑出重複的句子。</div>` : ''}
  ...
  // 重畫是為了讓音軌那段說明跟著出現／消失；asrPanel 會把 selected 補回去。
