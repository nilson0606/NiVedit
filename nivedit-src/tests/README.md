...
music-example.e2e.cjs：64 項（配樂 v2），涵蓋本機／網站、曲目搜尋與分類、預覽不改專案、本機音檔、失敗重試、40 首 × 40 秒音訊解碼、720p／4K 提醒、範例取消、首次另存、後續儲存、另存另一份、重新載入原始範例及使用者檔案。原生 picker 使用測試 handle，其餘保存流程實際執行。設定 NIVEDIT_HTML、NIVEDIT_CHROME、NIVEDIT_FIX、NIVEDIT_WORKSPACE 後用 Node 執行。
