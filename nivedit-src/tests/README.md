...
十六支計數式測試共 730 項（含終點對位 90 項、motion 67 項、字幕 56 項、圖層 51 項、旋轉／儲存 71 項、片段選項 71 項、雙軌 45 項）；另有 GIF 斷言式專項，以及兩支 i18n 診斷腳本。

## v12.3 素材庫與本次驗證

新增 effect-library.e2e.cjs：file:// 與 /NiVedit/ 網站子路徑、240 組縮圖與分類搜尋、預覽延遲載入、自己的 PNG／GIF 匯入、日夜／英文繁中、對話框鍵盤、切割相位、復原重做、無素材庫請求時重新開專案、失敗重試、記憶體釋放。最後逐一核對 240 組 payload 雜湊、Edge 解碼畫格數與總時間。

本次以 Windows Edge 154 執行既有 16 支計數式測試：729／730；GIF 專項與兩支語言診斷完成。唯一差異是 layers 的標題像素計數門檻：預覽 84、H.264 解碼 67。未改過的 v12.2 也得出完全相同結果（20.24% 差異，原門檻 <20%），保留原門檻及失敗紀錄，不宣稱全綠。未改動影片渲染或匯出程式。

兩個原始素材（shorts-029、wild-001）尾幀前重複寫了 GIF 檔頭，Edge 會提早停止。生成器只移除區塊邊界上的相同檔頭，保留所有畫格資料、色盤、disposal 與停留時間；以 Pillow 逐幀比對 RGBA 位元組相同，原始素材不變。catalog 同時記錄 sourceSha256 與修正後 sha256，供追查。
...
**版本斷言**：十六支測試都有 `$('#verTag').textContent === VER`，`VER` 來自
`tests/_ver.cjs`。改版號**只改那一個檔**，測試本身不用動。這一項是刻意的 ——
它同時在確認「你測到的真的是剛建置出來的那份」。

斷言的**標籤**固定是「版本」／「version」，不要把版號寫進標籤
（v9.8 以前有的標籤還停在「版本 v9.1」，跟實際斷言值早就不一致）。

> **v11.9 以前是用 `sed -i "s/v11\.7/v11.8/g" tests/*.cjs` 改的，那是個錯誤做法。**
> 它會把**註解裡的版號一起改掉**：每發一版，「v11.1 存檔改成先寫暫存檔再改名」
> 這類歷史紀錄就往前跳一版，久了整批都指著當下的版號。發現時已經追不回原值，
> 註解裡現有的版號**不可信**，要考古還原（列在交接說明的待辦裡）。
> 不要再用 `sed` 改版號。

`gif.e2e.cjs` 的版本檢查跑在**瀏覽器裡**（`waitForFunction`），所以 `VER` 要當參數傳進去，
不能直接在回呼裡引用 —— 那是 Node 的變數，頁面看不到。
...
| `subtitle-scope.e2e.cjs` | 空白上軌新增、上下軌列表及批次隔離、原生／備援 SRT、清空與復原、移軌、時間軸選取跟隨、存讀與翻譯、「＋ 字幕」兩選一、AI 字幕範圍選音軌 | 50 |
| `subtitle-edit.e2e.cjs` | 新增聚焦與選取、關閉同步、點選不誤拖、重開定位、時間／換軌重疊提示、存讀、排序不改原資料、合併、空白建立與翻譯 | 20 |
| `rotation-save.e2e.cjs` | 五類素材 ±360°、動畫像素、專案重開、原生 picker adapter／真實 OPFS 寫入、失敗復原、改名、儲存 MP4 解碼、下載備援、進度視窗「完成」不留上一輪的收尾動作 | 71 |
...
| `volume-curve.e2e.cjs` | 配樂音量曲線：插值、**預覽與匯出一致**、分割、UI、真的匯出一次、屬性欄打字時重畫不丟焦點 | 25 |
...
| `clip-options.e2e.cjs` | 每段內扣／外加、獨立淡化、即時更新、換軌／復原／分割、遷移／存讀、MP4 及原聲／配樂頻率分量、雙軌字幕連動亮度、裁切「現在／歸零／到底」、圖片/影片的按鈕用詞、點一下不記復原點、Ctrl+Z 的焦點情境 | 71 |
...
- `縮放真的把內容撐得比視窗寬（不然上一條驗不到東西）` ——
  **沒有這一條，上一條可能因為「內容根本沒撐開」而白過**。
  這是 v11.7 那次容器測不出來的直接教訓：測試資料要撐到會出事的那個區間。

## v12.4：背景音樂與內建範例

music-example.e2e.cjs：62 項，涵蓋本機／網站、曲目搜尋與分類、預覽不改專案、本機音檔、失敗重試、40 首音訊解碼、720p／4K 提醒、範例取消、首次另存、後續儲存、另存另一份、重新載入原始範例及使用者檔案。原生 picker 使用測試 handle，其餘保存流程實際執行。設定 NIVEDIT_HTML、NIVEDIT_CHROME、NIVEDIT_FIX、NIVEDIT_WORKSPACE 後用 Node 執行。

完整本機結果：qa-v12.4/regression/results.json；新功能結果：qa-v12.4/music-example-qa/results.json。標題像素容差的 Edge 154 既有差異見 v12.3 對照紀錄，沒有放寬測試閾值。
