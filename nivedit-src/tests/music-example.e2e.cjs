...
(async()=>{
 ...
 await p.locator('#muCategory').selectOption('');await p.locator('#muSearch').fill('piano');check(protocol+': instrument search',await p.locator('.mu-card').count()>0);
 await p.locator('#muSearch').fill('晨光');check(protocol+': Chinese title search',await p.locator('.mu-card').count()===1);
 ...
 check(protocol+': import retains duration/defaults, releases audition',await p.evaluate(()=>Math.abs(A.musics[0].el.duration-25)<.1&&A.musics[0].vol===.5&&A.musics[0].loop&&MU.url===null&&document.querySelector('#muAudio').paused));
 ...
 await p.locator('#muSearch').fill('Gentle closing');await p.locator('.mu-card').click();await p.locator('#muRetry').waitFor({state:'visible'});
 ...
 check('all 40 files decode, have unique hashes, stereo 25 seconds and non-silent unclipped audio',decoded.length===40&&new Set(decoded.map(x=>x.sha)).size===40&&decoded.every(x=>x.valid&&Math.abs(x.duration-25)<.002&&x.channels===2&&x.rms>.02&&x.peak<.96));
