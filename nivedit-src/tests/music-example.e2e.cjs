...
(async()=>{
 ...
 check(protocol+': new pack and download selected',await p.evaluate(()=>MUSIC_PACK.version==='v3'&&MUSIC_PACK.newTracks===60&&MUSIC_PACK.previousTracks===40&&MUSIC_CATALOG.every(x=>/^v[23]-/.test(x.id)&&x.duration===40)&&document.querySelector('#muPackDownload').download==='NiVedit_Music_101_40s_v3.zip'));
 check(protocol+': 60 new tracks in 12 genres, five per genre',await p.evaluate(()=>{const a=MUSIC_CATALOG.filter(x=>x.id.startsWith('v3-'));const genres=[...new Set(a.map(x=>x.genre))];return a.length===60&&genres.length===12&&genres.every(g=>a.filter(x=>x.genre===g).length===5)&&MUSIC_CATALOG.slice(0,60).every(x=>x.id.startsWith('v3-'));}));
