...
(async()=>{
 ...
 check(protocol+': 41 tracks, 40 new cues plus the example',await p.evaluate(()=>MUSIC_CATALOG.length===41&&MUSIC_CATEGORIES.length===4&&MUSIC_CATEGORIES.every(c=>MUSIC_CATALOG.filter(e=>e.category===c.id).length===(c.id==='bright'?11:10))));
 check(protocol+': new pack and download selected',await p.evaluate(()=>MUSIC_PACK.version==='v2'&&MUSIC_CATALOG.every(x=>x.id.startsWith('v2-')&&x.duration===40)&&document.querySelector('#muPackDownload').download==='NiVedit_Music_41_40s_v2.zip'));
 ...
 check('all 41 files decode, have unique hashes, stereo 40 seconds and non-silent unclipped audio',decoded.length===41&&new Set(decoded.map(x=>x.sha)).size===41&&decoded.every(x=>x.valid&&Math.abs(x.duration-40)<.002&&x.channels===2&&x.rms>.02&&x.peak<.96));
