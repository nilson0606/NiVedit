...
(async()=>{
 ...
 check(protocol+': 101 tracks, 60 new plus 40 previous cues and the example',await p.evaluate(()=>MUSIC_CATALOG.length===101&&MUSIC_CATEGORIES.length===4&&MUSIC_CATEGORIES.every(c=>MUSIC_CATALOG.filter(e=>e.category===c.id).length===(c.id==='bright'?26:25))));
 check(protocol+': new pack and download selected',await p.evaluate(()=>MUSIC_PACK.version==='v4'&&MUSIC_PACK.newTracks===60&&MUSIC_PACK.previousTracks===40&&MUSIC_CATALOG.every(x=>/^v[24]-/.test(x.id)&&x.duration===40)&&document.querySelector('#muPackDownload').download==='NiVedit_Music_101_40s_v4.zip'));
 check(protocol+': 60 rearranged tracks, 12 moods, varied styles and forms',await p.evaluate(()=>{const a=MUSIC_CATALOG.filter(x=>x.id.startsWith('v4-'));const moods=[...new Set(a.map(x=>x.mood))];return a.length===60&&moods.length===12&&moods.every(g=>a.filter(x=>x.mood===g).length===5)&&new Set(a.map(x=>x.genre)).size>=24&&new Set(a.map(x=>x.structure)).size===60&&MUSIC_CATALOG.slice(0,60).every(x=>x.id.startsWith('v4-'));}));
 ...
 await p.evaluate(()=>openMusicLibrary());await p.evaluate(()=>{window.savedStem=MUSIC_CATALOG.find(e=>e.en==='Until next time').stem;MUSIC_CATALOG.find(e=>e.en==='Until next time').stem='missing-test-music';});
 ...
 await p.evaluate(()=>MUSIC_CATALOG.find(e=>e.en==='Until next time').stem=savedStem);await p.locator('#muRetry').click();await p.waitForFunction(()=>MU.file);
 ...
 check('all 101 files decode, have unique hashes, stereo 40 seconds and non-silent unclipped audio',decoded.length===101&&new Set(decoded.map(x=>x.sha)).size===101&&decoded.every(x=>x.valid&&Math.abs(x.duration-40)<.002&&x.channels===2&&x.rms>.02&&x.peak<.96));
