...
(async()=>{
 ...
 chk('version',await p.textContent('#verTag')==='v10.5');
 ...
 // v10.5：下軌那一組排在上面，整條時間軸的 L 編號才由上往下遞增（L1,L1,L2,L2,L3…）。
 ...
 chk('timeline uses slightly shorter v9.1 default',layout.tl===440);
 ...
 // ── v10.5：字幕自由定位（x＝字幕框水平中心、y＝字幕框底緣，整軌共用）──
 ...
 chk('Chinese subtitle labels restored',(await p.textContent('#trkVideo')).includes('字幕上軌'));
 ...
 // ── v10.5：字幕綁著某一段影片，那一段搬到哪就跟到哪（時間＋軌道）──
 ...
 chk('片段換到上軌，字幕也跟著換到字幕上軌（否則會被自己那段影片蓋住）',
 ...
 // 那些上軌字幕會依時間綁到下軌的影片。若寫成「字幕永遠跟影片同軌」，
 ...
 chk('一條影片軌＋兩條字幕軌：上軌字幕原地不動',scopeSafe.same===1);
 chk('同軌左右拖影片，不會把上軌字幕拖下來',scopeSafe.afterMove===1);
