...
Only the Volume curve section heading and help text are larger and use higher-contrast colors in both dark and light modes. Other audio-track text keeps its original appearance. Audio behavior is unchanged.
...
The checked-in HTML is the verified v9.3 build. GitHub Pages serves the `docs` directory on `main`. After pushing a rebuilt `docs/index.html`, Pages updates automatically.
...
這是 NiVedit v9.3 的網站與原始碼。建議使用桌面版 Microsoft Edge；可切換繁體中文與日／夜模式，預設夜間模式。
